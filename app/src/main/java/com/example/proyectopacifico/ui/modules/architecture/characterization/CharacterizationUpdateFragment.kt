...
import com.example.proyectopacifico.databinding.FragmentCharacterizationUpdateBinding
import com.example.proyectopacifico.databinding.FragmentObservationUpdateBinding
...
import com.example.proyectopacifico.ui.modules.architecture.observation.ObservationUpdateFragmentArgs
import com.google.android.material.snackbar.Snackbar


class CharacterizationUpdateFragment : Fragment(R.layout.fragment_characterization_update) {
    private lateinit var binding: FragmentCharacterizationUpdateBinding

    private val args by navArgs<CharacterizationUpdateFragmentArgs>()
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding = FragmentCharacterizationUpdateBinding.bind(view)
    ...
    private fun clicks() {
        ...
        binding.btnUpdateCharacterisation.setOnClickListener { obtainAnswers() }
    }
    ...
    private fun obtainAnswers() {
        if (false in validateForm()) {
                ...
                if (optionList[i].second == "editText") {
                    if ((optionList[i].first as EditText).text.toString()
                        ...
                        AnswersEntity(
                            ...
                            args.answersAndQuestions.answersAndQuestions[0].place_id,
            ...
            deleteAnswers(answersList)
        }
    }

    private fun deleteAnswers(answersList: MutableList<AnswersEntity>) {
        var cantDeleteAnswers = 0
        args.answersAndQuestions.answersAndQuestions.forEach {
            viewLifecycleOwner.lifecycleScope.launchWhenCreated {
                viewLifecycleOwner.lifecycle.repeatOnLifecycle(Lifecycle.State.STARTED) {
                    viewModelAnswers.deleteAnswer(
                        AnswersEntity(
                            it.id_answer,
                            it.question_answer_id,
                            it.option_question_id,
                            it.place_id,
                            it.open_answer,
                        )
                    )
                        .collect { r ->
                            when (r) {
                                ...
                                is Result.Success -> {
                                    cantDeleteAnswers++
                                    if (cantDeleteAnswers == args.answersAndQuestions.answersAndQuestions.size) {
